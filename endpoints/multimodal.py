"""
Multimodal API routes for image, music, and video generation
Using Google Vertex AI and Gemini APIs
"""
import base64
import json
import logging
...
try:
    ...
    logger.warning(f"Multimodal services not configured: {e}")
except Exception as e:
    logger.error(f"Error initializing multimodal services: {e}")


@multimodal_bp.before_request
def check_config():
    if not config:
        return jsonify({"error": "Multimodal API not configured. Please set required environment variables."}), 503


@multimodal_bp.route("/api/multimodal", methods=["GET"])
def multimodal_guide():
    guide = (
...
@multimodal_bp.route("/api/generate-image", methods=["POST"])
...
@multimodal_bp.route("/api/generate-music", methods=["POST"])
...
@multimodal_bp.route("/api/video/start", methods=["POST"])
...
@multimodal_bp.route("/api/video/status/<operation_id>", methods=["GET"])
